...
{
    public class LegacyTimestampTzHandlerFactory : NpgsqlTypeHandlerFactory<Instant>
    {
        ...
        public override NpgsqlTypeHandler<Instant> Create(PostgresType postgresType, NpgsqlConnector conn)
            => conn.DatabaseInfo.HasIntegerDateTimes
                ? new LegacyTimestampTzHandler(postgresType, conn.Settings.ConvertInfinityDateTime)
