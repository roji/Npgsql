...
#pragma warning restore CA2252
    ...
    /// <summary>
    /// Constructor used for opening a cancellation connection only.
    /// </summary>
    ...
    internal async Task Open(NpgsqlTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await PrepareDataSourceStatements(async, cancellationToken);
...
#pragma warning restore CA2252
    ...
    async Task PrepareDataSourceStatements(bool async, CancellationToken cancellationToken = default)
    {
        if (_dataSource.PreparedStatements.Count == 0)
            return;

        // Prepare all the statements registered on the data source
        foreach (var preparedStatement in _dataSource.PreparedStatements)
        {
            // TODO: Parameters
            await WriteParse(
                preparedStatement.Sql, preparedStatement.StatementName, new List<NpgsqlParameter>(), async, cancellationToken);
            await WriteDescribe(StatementOrPortal.Statement, preparedStatement.StatementName, async, cancellationToken);
        }

        await WriteSync(async, cancellationToken);
        await Flush(async, cancellationToken);

        for (var i = 0; i < _dataSource.PreparedStatements.Count; i++)
        {
            // TODO: Error handling - include identifying information
            Expect<ParseCompleteMessage>(await ReadMessage(async), this);
            Expect<ParameterDescriptionMessage>(await ReadMessage(async), this);

            var msg = await ReadMessage(async);
            switch (msg.Code)
            {
            case BackendMessageCode.RowDescription:
                // Clone the RowDescription for use with the prepared statement (the one we have is reused by the connection)
            ...
            case BackendMessageCode.NoData:
                // Leave the RowDescription null
                break;
            default:
                throw UnexpectedMessageReceived(msg.Code);
            }
